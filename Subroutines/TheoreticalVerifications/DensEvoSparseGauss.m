function [E_new_rep, V_new_rep, E_new_cav, V_new_cav] = DensEvoSparseGauss(type, rep, cav, rho, alpha, Delta, E, V, varargin)
% Density evolution iteration for a gaussian signal with zero mean and unit
% variance, in the full or seeded case (under the NISHIMORI condition for seeded case) (type)
...
if (strcmp(type, 'full') )
    ...
    if (cav == 1)
        ...
        E_new_cav = (1 - rho) .* integral(@(s) G(s) .* max(1e-18, f_a((Delta + V) ./ alpha,s .* sqrt((E + Delta) ./ alpha),rho).^2),minn,maxx,'AbsTol',1e-20) + rho .* (1 - 2 .* alpha ./ (Delta + V) .* int_ + integral(@(s) G(s) .* f_a((Delta + V) ./ alpha,s .* sqrt((E + Delta) ./ alpha + 1),rho).^2,minn,maxx,'AbsTol',1e-20) );
    end
...
elseif (strcmp(type, 'seeded') )
    ...
    for c = 1 : numBlockC;
        ...
        int_ = integral(@(z) exp(-0.5 .* z.^2) ./ sqrt(2 .* pi) .* z.^2 ./ (rho + (1 - rho) .* exp(-0.5 .* z.^2 .* m(c) ) .* sqrt(m(c) + 1) ), minn, maxx, 'AbsTol', 1e-20);
        E_new_cav(c) = rho .* (1 - rho .* m(c) ./ (m(c) + 1) .* int_);
    end
